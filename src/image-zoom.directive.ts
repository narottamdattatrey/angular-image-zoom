...
export class ImageZoom implements OnInit, OnDestroy, OnChanges {
  ...
  private setImageBackgroundPosition() {
    let x: number = (((this._currentX - this._containerOffsetX) - this._elementOffsetX)
      * this._widthRatio - this.lensWidth / 2 - 150) * -1;
  ...
  private setWindowPosition() {
    if (this.lensStyle.toUpperCase() === 'LENS') {
      this.imageZoomContainer.setWindowPosition(this._currentX - (this.lensWidth / 2)
       - this.lensBorder - this._containerOffsetX, this._currentY - (this.lensHeight / 2)
       - this.lensBorder - this._containerOffsetY);
      // Account for lens border shifting image down and to the right
  ...
  public onMouseleave(event: MouseEvent) {
    ...
    if (y <= this.img.getBoundingClientRect().top || y >= (this.img.getBoundingClientRect().top +
     this.img.height) || x <= this.img.getBoundingClientRect().left || x >= (this.img.getBoundingClientRect().left + this.img.width)) {
      if (this._mouseEnterDebounce !== 0) {
        clearTimeout(this._mouseEnterDebounce);
      }
      if (this.isZooming) {
